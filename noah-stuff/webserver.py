...
def get_preview():
    ...
    if(len(args) != 1):
        ...
        if('userid' not in args.keys):
            ...
            #Add distance from query point to return
            latest = {}
            for (id,userfromid,usertoid,message, timestamp) in enumerate(ret_val):
                time = datetime.datetime(timestamp)
                dict_insert = {'id': id, 'message': message, 'timestamp':timestamp }
                otheruser = None
                if(userfromid==userid):
                    otheruser = usertoid
                else:
                    otheruser = userfromid
                if(otheruser in latest.keys):
                    if(timestamp >= latest[otheruser][timestamp]):
